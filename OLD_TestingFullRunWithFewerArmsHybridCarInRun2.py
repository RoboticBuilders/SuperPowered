...
def _run6():
    ...
    gyroStraight(speed=25, distance=26, targetAngle=162)
    ...
    gyroStraight(distance=25, speed = 20, backward = True, targetAngle = 162)
...
def _dorun6ToyFactory():
    ...
    #changed to 16 on 12/30/2022
    gyroStraight(distance=16, speed = 35, backward = True, targetAngle = angle)

    angle = -135
    _turnToAngle(targetAngle = angle, speed = 20, slowTurnRatio = 0.9)
    #changed to 11 on 12/21/2022
    #changed to 10 on 12/21/2022
    gyroStraight(distance=15, speed = 35, backward = True, targetAngle = angle)
...
def _fasterRun1():
    ...
    def _pickUpRechargeableBattery():
        ...
        #primeHub.motion_sensor.reset_yaw_angle()
        gyroStraight(distance=5, speed = 45, backward = False, targetAngle = angle)
    ...
    def goHomeFromechargeableBattery():
        # Backoff from the rechargeable batter to be able to turn
        angle = 135
        _turnToAngle(targetAngle = angle, speed = 35, slowTurnRatio = 0.9)
        gyroStraight(distance=70, speed = 80, backward = False, targetAngle = angle)
...
def _fasterRun2():
    ...
    def _doRechargablebattery():
        ...
        gyroStraight(distance=43, speed=straightSpeed,targetAngle=angle,backward=False)

        # Turn towards rechargeable battery/ToyFactory and flush with the toy factory
        angle = -135
        angle, correction = calculateReducedTargetAngleAndCorrection(angle, correction)
        _turnToAngle(targetAngle = angle, speed = 35, slowTurnRatio = 0.1)
        flushForTime(speed=35, timeInSeconds=1)
        #gyroStraight(distance=25, speed=straightSpeed,targetAngle=angle,backward=False)

        # Backoff from the toy factory
        gyroStraight(distance=5, speed=straightSpeed, targetAngle=angle, backward=True)
        
        # Lift the arm to drop off the units
        moveArm(degrees = 120, speed = -75, motor = motorF)
    ...
    def _hybridCar():
        ...
        gyroStraight(distance=10, speed = 20, backward = True, targetAngle = angle)
        moveArm(degrees = 120, speed = 75, motor = motorF)
    ...
    def _goHome():
        ...
        if (ROBOT == "A"):
            gyroStraight(distance=90, speed = 80, backward = False, targetAngle = angle, multiplier=2.0, gradualAcceleration=False, slowDown=False)
        else:
